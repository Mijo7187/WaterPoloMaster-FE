...
import { CodeSandboxOutlined, EuroCircleOutlined, LockOutlined } from "@ant-design/icons";
...
export const UxStockInformation: FC<IUxStockInformationProps> = observer(
  ({ stockAmount, reservedAmount, incomeAmount }) => {
    const STOCK_INFO_OPTIONS: StockOptions[] = [
      {
        icon: <CodeSandboxOutlined style={{ fontSize: 20, fontWeight: "bold" }} />,
        label: "Stock",
      ...
      {
        icon: <EuroCircleOutlined style={{ fontSize: 20, fontWeight: "bold" }} />,
        label: "Income amount",
    ...
    return (
      <Flex className="w-100" justify={"space-around"} gap={16} align={"middle"}>
        {STOCK_INFO_OPTIONS.map((item: StockOptions, index: number) => (
          <UxCard testId={`stock-info-${index}`} className="w-100" key={index}>
